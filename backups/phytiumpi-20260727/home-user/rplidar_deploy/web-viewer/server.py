...
from depth_voxel_map import DepthVoxelMap
...
VOICE_NAVIGATION_PATH = os.environ.get(
    "VOICE_NAVIGATION_PATH",
    "/var/lib/robot-data/voice_navigation_command.json",
)
NEW_MAP_REQUEST_PATH = Path(os.environ.get(
    "NEW_MAP_REQUEST_PATH",
    "/var/lib/robot-requests/new_map_request.json",
))
NEW_MAP_STATUS_PATH = Path(os.environ.get(
    "NEW_MAP_STATUS_PATH",
    "/var/lib/robot-requests/new_map_status.json",
))
NEW_MAP_REQUEST_LOCK = threading.Lock()
...
def read_voice_navigation_state(path=VOICE_NAVIGATION_PATH, maximum_age=45.0):
    """Read a preview-only voice destination without executing any navigation."""
    state = read_acoustic_state(path, maximum_age)
    if not state.get("online"):
        return state
    destination = str(state.get("destination", "")).strip()
    if not destination or state.get("preview_only") is not True:
        return {"online": False}
    state["destination"] = destination[:120]
    state["utterance"] = str(state.get("utterance", ""))[:240]
    return state


def queue_new_map_request():
    """Queue a constrained user-service request; never invoke shell commands here."""
    with NEW_MAP_REQUEST_LOCK:
        status = read_acoustic_state(NEW_MAP_STATUS_PATH, maximum_age=240.0)
        if status.get("online") and status.get("state") == "running":
            raise ValueError("new-map preparation is already running")
        if NEW_MAP_REQUEST_PATH.exists():
            raise ValueError("new-map request is already queued")
        now = time.time()
        payload = {
            "action": "new_site",
            "confirm": True,
        ...
        NEW_MAP_REQUEST_PATH.parent.mkdir(parents=True, exist_ok=True)
        temporary = NEW_MAP_REQUEST_PATH.with_suffix(NEW_MAP_REQUEST_PATH.suffix + ".tmp")
        with temporary.open("w", encoding="utf-8") as output:
            json.dump(payload, output, separators=(",", ":"))
            output.flush()
            os.fsync(output.fileno())
        os.replace(temporary, NEW_MAP_REQUEST_PATH)
        return {
            "ok": True,
            "sequence": payload["sequence"],
            "preview_only": True,
            "message": "new-map request queued",
        }
...
class LatestData(Node):
    def __init__(self):
        ...
        self.semantic_map = SemanticMapStore(
            ...
            minimum_quality=float(
                os.environ.get("SEMANTIC_MINIMUM_QUALITY", "0.25")
            ),
            maximum_observation_age_s=float(
                os.environ.get("SEMANTIC_MAXIMUM_OBSERVATION_AGE_S", "2.5")
            ),
            relocation_observations=int(
                os.environ.get("SEMANTIC_RELOCATION_OBSERVATIONS", "3")
            ),
            site_id=os.environ.get("SEMANTIC_SITE_ID") or None,
        )
        self.depth_voxel_map = DepthVoxelMap(
            voxel_size_m=float(os.environ.get("DEPTH_VOXEL_SIZE_M", "0.10")),
            pixel_stride=int(os.environ.get("DEPTH_VOXEL_PIXEL_STRIDE", "8")),
            minimum_depth_m=float(os.environ.get("DEPTH_VOXEL_MIN_DEPTH_M", "0.25")),
            maximum_depth_m=float(os.environ.get("DEPTH_VOXEL_MAX_DEPTH_M", "4.0")),
            minimum_z_m=float(os.environ.get("DEPTH_VOXEL_MIN_Z_M", "0.04")),
            maximum_z_m=float(os.environ.get("DEPTH_VOXEL_MAX_Z_M", "1.8")),
            minimum_observations=int(
                os.environ.get("DEPTH_VOXEL_MINIMUM_OBSERVATIONS", "3")
            ),
            stale_after_s=float(os.environ.get("DEPTH_VOXEL_STALE_AFTER_S", "45")),
            maximum_voxels=int(os.environ.get("DEPTH_VOXEL_MAXIMUM", "12000")),
            maximum_output_voxels=int(
                os.environ.get("DEPTH_VOXEL_MAXIMUM_OUTPUT", "6000")
            ),
            robot_half_length_m=float(
                os.environ.get("ROBOT_FOOTPRINT_HALF_LENGTH_M", "0.20")
            ),
            robot_half_width_m=float(
                os.environ.get("ROBOT_FOOTPRINT_HALF_WIDTH_M", "0.25")
            ),
        )
        self.depth_voxel_last_sequence = -1
        self.depth_voxel_reason = "waiting for aligned depth and map TF"
        ...
        self.rgb_mjpeg_period = 1.0 / max(
            1.0, min(15.0, float(os.environ.get("CAMERA_MJPEG_FPS", "6")))
        )
        ...
        self.navigation_path_sequence = 0
        ...
        self.world_poses = {}
        self.world_tf_max_age_s = float(
            os.environ.get("WORLD_TF_MAXIMUM_AGE_S", "1.5")
        )
        ...
        self.navigation_goal_publisher = self.create_publisher(
            PoseStamped, "/navigation/preview_goal", 10
        )
        self.navigation_clear_publisher = self.create_publisher(
            Empty, "/navigation/preview_clear", 10
        )
        self.navigation_path_subscription = self.create_subscription(
            NavPath, "/navigation/preview_path", self.on_navigation_path, map_qos
        )
        self.navigation_status_subscription = self.create_subscription(
            String, "/navigation/preview_status", self.on_navigation_status, map_qos
        )
        self.tf_buffer = Buffer()
        self.tf_listener = TransformListener(self.tf_buffer, self)
        self.pose_timer = self.create_timer(0.1, self.update_world_poses)
        self.depth_voxel_timer = self.create_timer(
            float(os.environ.get("DEPTH_VOXEL_PERIOD_S", "2.0")),
            self.integrate_depth_voxel_frame,
        )
    ...
    def snapshot(self, compact=False):
        ...
        with self.lock:
            now = time.monotonic()
            depth_voxel_map = self.depth_voxel_map.snapshot(
                now, include_points=not compact
            )
            depth_voxel_map["reason"] = self.depth_voxel_reason
            ...
            return {
                ...
                "depth_voxel_map": depth_voxel_map,
                "acoustic": acoustic,
                "hardware": hardware,
    ...
    def _store_camera_image(self, kind, msg, aliases=()):
        ...
        with self.camera_condition:
            ...
            for frame_kind in (kind, *aliases):
                self.camera_sequences[frame_kind] += frame_kind != kind
                frame_value = dict(frame)
                frame_value["sequence"] = self.camera_sequences[frame_kind]
                self.camera_frames[frame_kind] = frame_value
                self.camera_times[frame_kind].append(received_at)
            for frame_kind in (kind, *aliases):
                if self.camera_seen[frame_kind]:
                    continue
                self.camera_seen[frame_kind] = True
                labels = {"rgb": "RGB", "depth": "深度", "aligned_depth": "对齐深度"}
                self._add_event_locked(f'{labels.get(frame_kind, frame_kind)}图像流已连接')
    ...
    def on_aligned_depth_image(self, msg):
        self._store_camera_image("aligned_depth", msg, aliases=("depth",))
    ...
    def on_targets_3d(self, msg):
        ...
        if not isinstance(targets, dict):
            return
        stamp = targets.get("stamp") or {}
        try:
            observed_at = float(stamp.get("sec", 0)) + float(
                stamp.get("nanosec", 0)
            ) * 1e-9
        except (TypeError, ValueError):
            observed_at = 0.0
        ...
        self.semantic_map.ingest(
            targets.get("targets") or [],
            observed_at=observed_at,
            source_context=targets,
        )
        with self.lock:
            self.targets_3d = targets

    def integrate_depth_voxel_frame(self):
        now = time.monotonic()
        with self.lock:
            frame = self.camera_frames.get("aligned_depth")
            info = self.aligned_camera_info
            rgb = self.camera_frames.get("rgb")
            if frame is None or info is None:
                self.depth_voxel_reason = "waiting for aligned depth and camera info"
                return
            if frame["sequence"] == self.depth_voxel_last_sequence:
                return
            self.depth_voxel_last_sequence = frame["sequence"]
            frame = dict(frame)
            info = dict(info)
            rgb = None if rgb is None else dict(rgb)

        if now - frame["received_at"] >= 2.0:
            with self.lock:
                self.depth_voxel_map.reject_frame()
                self.depth_voxel_reason = "aligned depth frame is stale"
            return
        frame_id = frame.get("frame_id")
        stamp_ns = int(frame.get("stamp_ns", 0))
        if not frame_id or stamp_ns <= 0:
            with self.lock:
                self.depth_voxel_map.reject_frame()
                self.depth_voxel_reason = "aligned depth frame has no timestamp"
            return
        ...
        try:
            ...
            with self.lock:
                self.depth_voxel_map.reject_frame()
                self.depth_voxel_reason = f"map TF unavailable: {error}"
            return
        ...
        if transform_stamp_ns <= 0 or abs(transform_stamp_ns - stamp_ns) > 250_000_000:
            with self.lock:
                self.depth_voxel_map.reject_frame()
                self.depth_voxel_reason = "map TF is stale for the depth timestamp"
            return
        ...
        if (
            rgb is not None
            and rgb["width"] == frame["width"]
            and rgb["height"] == frame["height"]
            and abs(int(rgb.get("stamp_ns", 0)) - stamp_ns) <= 400_000_000
        ):
            rgb_data = rgb["data"]
        ...
        with self.lock:
            map_context = None if self.map_frame is None else dict(self.map_frame)
            base_pose = self.world_poses.get("base_link")
            robot_pose = None if base_pose is None else {
                "position": dict(base_pose["position"]),
                "quaternion": dict(base_pose["quaternion"]),
            }
            lidar_points = []
            laser_pose = self.world_poses.get("laser")
            if self.scan is not None and laser_pose is not None:
                laser_position = laser_pose["position"]
                laser_quaternion = laser_pose["quaternion"]
                for sample in self.scan["points"]:
                    angle = sample["a"]
                    distance = sample["r"]
                    rotated = self._rotate_world_vector(
                        (
                    ...
                    lidar_points.append((
                        laser_position["x"] + rotated[0],
                        laser_position["y"] + rotated[1],
                    ))
        accepted = self.depth_voxel_map.integrate(
            frame["data"],
            frame["width"],
            frame["height"],
            frame["encoding"],
        ...
        with self.lock:
            self.depth_voxel_reason = f"integrated {accepted} filtered depth points"
    ...
    def navigation_snapshot(self):
        with self.lock:
            payload = dict(self.navigation_status)
            payload["preview_only"] = True
            payload["goal"] = (
                None if self.navigation_goal is None else dict(self.navigation_goal)
            )
            if self.navigation_path is None:
                payload["path"] = None
            else:
                payload["path"] = {
                    "frame": self.navigation_path["frame"],
                    "points": list(self.navigation_path["points"]),
                    "sequence": self.navigation_path["sequence"],
                    "age_ms": round(
                        (time.monotonic() - self.navigation_path["received_at"]) * 1000
                    ),
                }
            return payload

    def request_navigation_preview(self, x, y):
        x = float(x)
        y = float(y)
        if not math.isfinite(x) or not math.isfinite(y):
            raise ValueError("navigation goal must be finite")
        with self.lock:
            frame = self.map_frame
            pose = self.radar_pose
            if frame is None:
                raise ValueError("SLAM map is not available")
            yaw = frame["origin_yaw"]
            cos_yaw = math.cos(yaw)
            sin_yaw = math.sin(yaw)
            dx = x - frame["origin_x"]
            dy = y - frame["origin_y"]
            grid_x = int(
                math.floor((cos_yaw * dx + sin_yaw * dy) / frame["resolution"])
            )
            grid_y = int(
                math.floor((-sin_yaw * dx + cos_yaw * dy) / frame["resolution"])
            )
            if not 0 <= grid_x < frame["width"] or not 0 <= grid_y < frame["height"]:
                raise ValueError("navigation goal is outside the current map")
            occupancy = frame["data"][grid_y * frame["width"] + grid_x]
            if occupancy == 255:
                raise ValueError("navigation goal is in unknown map space")
            if occupancy >= 65:
                raise ValueError("navigation goal is occupied")
            heading = 0.0
            if pose is not None:
        ...
        message.pose.orientation.z = math.sin(heading * 0.5)
        message.pose.orientation.w = math.cos(heading * 0.5)
        self.navigation_goal_publisher.publish(message)
        self.add_event(f"导航预览目标：({x:.2f}, {y:.2f}) m")
        return {"ok": True, "goal": goal, "preview_only": True}

    def clear_navigation_preview(self):
        with self.lock:
            self.navigation_goal = None
            self.navigation_path = None
            self.navigation_status = {
        ...
        self.navigation_clear_publisher.publish(Empty())
        self.add_event("导航预览路线已清除")
        return {"ok": True, "preview_only": True}

    def on_navigation_path(self, msg):
        points = [
        ...
        with self.lock:
            self.navigation_path_sequence += 1
            self.navigation_path = {
                "frame": msg.header.frame_id or "map",
                "points": points,
                "sequence": self.navigation_path_sequence,
                "received_at": time.monotonic(),
            }
    ...
    def on_navigation_status(self, msg):
        try:
            ...
            status = {"state": "failed", "message": "导航状态格式无效"}
    ...
    def _transform_is_fresh(self, transform):
        ...
        return abs(now_ns - stamp_ns) <= int(self.world_tf_max_age_s * 1e9)
    ...
    def update_world_poses(self):
        ...
        for name, child_frame in self.world_frame_ids.items():
            try:
                ...
                continue
            if not self._transform_is_fresh(transform):
                continue
        ...
        if base_pose is not None:
            for name in ("gimbal_yaw", "gimbal_pitch", "camera"):
                ...
                try:
                    ...
                    continue
                if not self._transform_is_fresh(relative):
                    continue
...
WEB_LOGIC_JS = Path(__file__).with_name("web_logic.js").read_bytes()
INDEX_DEFLATE = zlib.compress(INDEX, 9)
THREE_JS_DEFLATE = zlib.compress(THREE_JS, 9)
WEB_LOGIC_JS_DEFLATE = zlib.compress(WEB_LOGIC_JS, 9)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/three.min.js":
            ...
            return
        if self.path == "/web_logic.js":
            self._send_compressible(
                WEB_LOGIC_JS,
                "text/javascript; charset=utf-8",
                (("Cache-Control", "no-store"),),
                WEB_LOGIC_JS_DEFLATE,
            )
            return
        ...
        if self.path == "/api/semantic-map":
            ...
            return
        if self.path == "/api/depth-voxel-map":
            with DATA.lock:
                payload = DATA.depth_voxel_map.snapshot()
                payload["reason"] = DATA.depth_voxel_reason
        ...
        if self.path == "/api/gimbal/status":
            ...
            return
        if self.path == "/api/voice/navigation":
            self._send_json_response(read_voice_navigation_state())
            return
        if self.path == "/api/site/status":
            self._send_json_response(
                read_acoustic_state(NEW_MAP_STATUS_PATH, maximum_age=300.0)
            )
            return
        ...
        if parsed.path == "/api/navigation":
            self._send_json_response(DATA.navigation_snapshot())
            return
    ...
    def _serve_rgb_mjpeg(self):
        ...
        try:
            while True:
                ...
                next_frame_at = max(
                    next_frame_at + DATA.rgb_mjpeg_period, time.monotonic()
                )
        except (BrokenPipeError, ConnectionResetError, OSError):
            return
        ...
        if self.path == "/api/site/new":
            try:
                ...
                if (
                    ...
                    or request.get("confirmation") != "新建地图"
                ):
                    raise ValueError("explicit new-map confirmation is required")
                response = queue_new_map_request()
                if response.get("ok"):
                    DATA.add_event("网页已提交新建地图任务")
                self._send_json_response(response, 202 if response.get("ok") else 409)
            except (OSError, ValueError, TypeError, json.JSONDecodeError) as error:
                self._send_json_response({"ok": False, "error": str(error)}, 400)
            return
        if self.path in ("/api/navigation/goal", "/api/navigation/clear"):
            try:
                if self.path.endswith("/clear"):
                    response = DATA.clear_navigation_preview()
                else:
                    content_length = int(self.headers.get("Content-Length", "0"))
                    if content_length <= 0 or content_length > 4096:
                        raise ValueError("invalid request length")
                    ...
                    response = DATA.request_navigation_preview(
        ...
        if self.path in ("/api/semantic-map/remove", "/api/semantic-map/clear"):
            try:
                ...
                if self.path.endswith("/remove"):
                    ...
                    new_site = request.get("new_site") is True
                    count = DATA.semantic_map.clear(new_site=new_site)
                    cleared_voxels = 0
                    if new_site:
                        cleared_voxels = DATA.depth_voxel_map.clear()
                        with DATA.lock:
                            DATA.depth_voxel_reason = "waiting for fresh depth data after site reset"
                    DATA.add_event(f"语义地图已清空：{count} 个地标")
                    self._send_json_response({
                        "ok": True,
                        "removed": count,
                        "new_site": new_site,
                        "cleared_voxels": cleared_voxels,
                        "site_id": DATA.semantic_map.site_id,
                    })
            except (OSError, ValueError, TypeError, json.JSONDecodeError) as error:
                self._send_json_response({"ok": False, "error": str(error)}, 400)
            return
