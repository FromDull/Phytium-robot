...
class QwenChatPolicy(QwenVisionPolicy):
    ...
    def _build_chat_payload(
        ...
        prompt = build_chat_prompt(user_text, compact_state, intent)
        if image_path:
            prompt += (
                "\n\n你已经收到一张机器人摄像头刚刚拍摄的真实图像。"
                "必须直接描述图中实际可见的人、物体和环境；不要只回答正在观察、准备观察或需要拍照。"
                "看不清的内容要明确说看不清，不要猜测。此次只做视觉回答，不执行运动。"
            )
        else:
            prompt += (
                "\n\n若用户没有提出机器人行动：普通问题正常聊天；"
                "用户表达难过、焦虑、害怕或疲惫时，先简短共情和安抚，再询问是否需要帮助，action设为none。"
            )
