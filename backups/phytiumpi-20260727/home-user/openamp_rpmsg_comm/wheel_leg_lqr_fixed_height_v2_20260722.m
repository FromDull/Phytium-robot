...
%% 1. 用户配置区：机械和固件参数（2026-07-22 加装云台/顶板/雷达）

cfg.m_total = 7.93;       % 新整车总质量，kg
cfg.m_wheels = 0.19;      % 两个旋转轮组总质量，kg（单个0.095 kg）
cfg.M_body = cfg.m_total - cfg.m_wheels;

cfg.wheel_radius_m = 0.03225;      % 车轮有效滚动半径，m
cfg.wheel_track_m = 0.355;         % 左右轮距，m（四状态俯仰模型暂不使用）
cfg.com_height_m = 0.155;          % 新固定腿高下，整车质心到轮轴高度，m

% 当前没有CAD直接给出的俯仰惯量，因此用两个长方体估算值给出范围：
%   low：质量集中在机体本体尺寸内；
%   high：质量均匀分布在整机最大包络内。
% 标称模型取二者中值。后续拿到CAD惯量或摆锤实测值后，只改
% cfg.body_pitch_inertia 即可。
cfg.robot_height_m = 0.18;
cfg.robot_length_m = 0.295;
cfg.body_height_m = 0.095;
cfg.body_length_m = 0.19;
cfg.body_pitch_inertia_low = cfg.M_body / 12 * ...
    (cfg.body_length_m^2 + cfg.body_height_m^2);
cfg.body_pitch_inertia_high = cfg.M_body / 12 * ...
    (cfg.robot_length_m^2 + cfg.robot_height_m^2);
cfg.body_pitch_inertia = 0.5 * ...
    (cfg.body_pitch_inertia_low + cfg.body_pitch_inertia_high);
cfg.gravity = 9.81;
...
cfg.pitch_trim_deg = 0.00;         % 前移重心实测需降低参考角，抑制启动前冲
cfg.pitch_rate_filter_hz = 40.0;
...
% 固件和执行器近似。根据实机5~10秒后振荡现象，先用保守的
% 两拍命令延迟、约90%的有效力矩和20 ms一阶响应做鲁棒筛选。
cfg.command_delay_samples = 2;
cfg.use_torque_quantization = true;

% 如果后续通过阶跃测试测出了力矩响应时间常数，再填入非零值。
% 当前先设为0，表示除了上面的一拍延迟外，执行器立即跟随命令。
cfg.actuator_time_constant_s = 0.020;
cfg.actuator_gain = 0.90;
cfg.left_torque_scale = 0.98;
cfg.right_torque_scale = 1.02;
...
%   F. position_hold_soft / position_hold_damped 用于姿态已稳定但轮轴
%      前移过大时的诊断候选；实机因先向前建立回拉姿态而触发轮速保护；
%   G. balanced_position 在 global_soft 与 robust_position 之间折中，
%      用于降低上述前冲，同时保留比 global_soft 更强的回零能力。
...
candidate_definitions = {
    ...
    'very_strong_position', 1200, 50, 8, 32, 0.75;
    'robust_position', 1200, 40, 6, 24, 1.00;
    'balanced_position', 1200, 40, 4, 16, 1.00;
    'position_hold_soft', 1200, 40, 8, 32, 1.00;
    'position_hold_damped', 1200, 40, 8, 40, 1.00;
    'global_soft',       1200, 40, 2,  10, 1.00;
};

% 选择要绘制详细曲线和输出部署命令的候选。修改这里即可切换。
% position_hold_soft 三轮实测均触发0.60 m/s轮速保护，说明更强K3会让
% 实机在建立回拉姿态前进一步前冲。global_soft/robust_position 在同一
% 0.60 deg trim 下分别偏后/偏前，下一轮用中档权重夹逼。
selected_candidate_id = 'balanced_position';

% 当前实机手动组合，仅用于对照。它不是同一组 Q/R 的 dlqr 完整结果。
runtime_reference_K = ...
    [-3.759674000, -0.360000000, -0.080000000, -0.300000000];
...
fprintf('惯量估算范围：               %.9f ~ %.9f kg*m^2\n', ...
        cfg.body_pitch_inertia_low, cfg.body_pitch_inertia_high);
fprintf('两个轮组总转动惯量：         %.9f kg*m^2\n', ...
        cfg.wheel_inertia);
fprintf('控制频率：                   %d Hz\n', cfg.control_hz);
fprintf('首轮 pitch trim：            %.6f deg\n', cfg.pitch_trim_deg);
...
fprintf('执行器时间常数/有效力矩：    %.3f s / %.3f\n', ...
        cfg.actuator_time_constant_s, cfg.actuator_gain);
...
fprintf('rprun balance-trim %.6f\n', cfg.pitch_trim_deg);
fprintf('rprun balance-gains %.9f %.9f %.9f %.9f\n', selected.K);
fprintf('rprun balance-config\n');

%% 7.1 俯仰惯量不确定性检查
%
% 新增云台、顶板和雷达后，长方体估算只能给出范围。这里用相同Q/R
% 分别计算低/标称/高惯量下的增益，便于判断CAD或实测惯量是否值得
% 进一步补充。实际部署仍只使用上面的标称 selected.K。

inertia_case_names = {'low', 'nominal', 'high'};
inertia_cases = [cfg.body_pitch_inertia_low, ...
                 cfg.body_pitch_inertia, ...
                 cfg.body_pitch_inertia_high];

fprintf('\n================ 俯仰惯量敏感性 ================\n');
for inertia_index = 1:numel(inertia_cases)
    inertia_value = inertia_cases(inertia_index);
    a_case = inertia_value + M * l^2;
    den_case = a_case * c - b^2;
    A_case = [0,                       1, 0, 0;
              c*M*g*l/den_case,       0, 0, 0;
              0,                       0, 0, 1;
             -b*M*g*l/den_case,       0, 0, 0];
    B_case = [0;
             -(c + b/r)/den_case;
              0;
              (b + a_case/r)/den_case];
    sys_case_d = c2d(ss(A_case, B_case, C, D), cfg.Ts, 'zoh');
    K_case = dlqr(sys_case_d.A, sys_case_d.B, ...
                  selected.Q, selected.R_tau);
    fprintf('%-8s I=%.9f kg*m^2  K=[%.6f %.6f %.6f %.6f]\n', ...
            inertia_case_names{inertia_index}, inertia_value, K_case);
end

%% 7.2 鲁棒性参数扫描
%
% 在低/标称/高惯量、1~3拍延迟、10~40 ms执行器响应和85~95%%有效
% 力矩范围内，比较主要候选的最坏情况。该扫描用于筛选实机首轮，
% 不替代保护绳和小角度测试。

robust_candidate_ids = {'global_soft', 'balanced_position', ...
                        'robust_position', ...
                        'position_hold_soft', 'position_hold_damped', ...
                        'log_target_soft'};
robust_inertias = [cfg.body_pitch_inertia_low, cfg.body_pitch_inertia, ...
                   cfg.body_pitch_inertia_high];
robust_delays = [1, 2, 3];
robust_time_constants = [0.010, 0.020, 0.040];
robust_actuator_gains = [0.85, 0.90, 0.95];

fprintf('\n================ 鲁棒性扫描 ================\n');
fprintf('每个候选扫描 %d 个执行器/惯量组合、%d 个场景。\n', ...
        numel(robust_inertias) * numel(robust_delays) * ...
        numel(robust_time_constants) * numel(robust_actuator_gains), ...
        scenario_count);

for robust_candidate_index = 1:numel(robust_candidate_ids)
    robust_id = robust_candidate_ids{robust_candidate_index};
    result_index = find(strcmp({results.id}, robust_id), 1);
    if isempty(result_index)
        error('鲁棒扫描候选不存在：%s', robust_id);
    end
    robust_K = results(result_index).K;
    robust_worst_pitch = 0.0;
    robust_worst_speed = 0.0;
    robust_worst_position = 0.0;
    robust_worst_saturation = 0.0;
    robust_diverged_count = 0;

    for robust_I = robust_inertias
        a_case = robust_I + M * l^2;
        den_case = a_case * c - b^2;
        A_case = [0,                 1, 0, 0;
                  c*M*g*l/den_case, 0, 0, 0;
                  0,                 0, 0, 1;
                 -b*M*g*l/den_case, 0, 0, 0];
        B_case = [0; -(c + b/r)/den_case; 0; (b + a_case/r)/den_case];
        sys_case = c2d(ss(A_case, B_case, C, D), cfg.Ts, 'zoh');
        model_case.Ad = sys_case.A;
        model_case.Bd = sys_case.B;

        for robust_delay = robust_delays
            for robust_tau = robust_time_constants
                for robust_gain = robust_actuator_gains
                    cfg_case = cfg;
                    cfg_case.command_delay_samples = robust_delay;
                    cfg_case.actuator_time_constant_s = robust_tau;
                    cfg_case.actuator_gain = robust_gain;
                    for robust_scenario_index = 1:scenario_count
                        robust_scenario = make_scenario( ...
                            scenario_definitions(robust_scenario_index, :));
                        robust_sim = simulate_firmware_controller( ...
                            model_case, cfg_case, robust_K, robust_scenario);
                        robust_worst_pitch = max(robust_worst_pitch, ...
                            robust_sim.max_pitch_deg);
                        robust_worst_speed = max(robust_worst_speed, ...
                            robust_sim.max_speed_m_s);
                        robust_worst_position = max(robust_worst_position, ...
                            max(abs(robust_sim.x(3, :))));
                        robust_worst_saturation = max( ...
                            robust_worst_saturation, robust_sim.saturation_percent);
                        robust_diverged_count = robust_diverged_count + ...
                            double(robust_sim.diverged);
                    end
                end
            end
        end
    end
    fprintf('%-18s worst pitch=%6.3f deg  speed=%6.3f m/s  ', ...
            robust_id, robust_worst_pitch, robust_worst_speed);
    fprintf('position=%6.3f m  saturation=%6.2f%%  diverged=%d/%d\n', ...
            robust_worst_position, robust_worst_saturation, ...
            robust_diverged_count, numel(robust_inertias) * ...
            numel(robust_delays) * numel(robust_time_constants) * ...
            numel(robust_actuator_gains) * scenario_count);
end
...
function sim = simulate_firmware_controller(model, cfg, K, scenario)
    ...
    for k = 1:(sample_count - 1)
        ...
        effective_delayed_torque = delayed_torque * cfg.actuator_gain * ...
            0.5 * (cfg.left_torque_scale + cfg.right_torque_scale);
        actuator_torque = actuator_torque + ...
            actuator_alpha * (effective_delayed_torque - actuator_torque);
        applied_total_torque(k) = actuator_torque;

        x(:, k + 1) = model.Ad * x(:, k) + ...
                      model.Bd * applied_total_torque(k);

        % 防止不稳定参数组合把数值仿真推到 Inf/NaN，污染鲁棒性排名。
        if any(~isfinite(x(:, k + 1))) || ...
                abs(x(1, k + 1)) > deg2rad(30) || ...
                abs(x(2, k + 1)) > 10 || ...
                abs(x(3, k + 1)) > 5 || ...
                abs(x(4, k + 1)) > 5
            diverged = true;
            x(:, k + 1:end) = repmat(x(:, k + 1), 1, ...
                                     sample_count - k);
            break;
        end
    end
